...
async def fetch_data(factory: str, year: int, db: Session = Depends(get_db)):
    try:
        ...
        for record in records:
            category, indicator = record.indicator.split(".", 1) if "." in record.indicator else ("默认", record.indicator)
            if category not in data_dict:
                data_dict[category] = {}
            data_dict[category][indicator] = {
    ...
    try:
        ...
        for category, indicators in data.data.items():
            for indicator, item in indicators.items():
                db_record = GovernanceQualitative(
