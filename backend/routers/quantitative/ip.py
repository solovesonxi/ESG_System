...
async def fetch_data(factory: str, year: int, db: Session = Depends(get_db),
    ...
    try:
        ...
        data_dict = {"patents_total": data.patents_total, "invention_total": data.invention_total,
            "invention_applications": data.invention_applications, "utility_model_total": data.utility_model_total,
            "design_total": data.design_total, "authorized_total": data.authorized_total,
            "new_patents_year": data.new_patents_year, "software_copyright_total": data.software_copyright_total,
...
async def submit_data(data: IPRSubmission, factory: str, year: int, db: Session = Depends(get_db),
    ...
    try:
        ...
        db_record = IPRData(factory=factory, year=year, patents_total=data.patents_total,
            invention_total=data.invention_total, invention_applications=data.invention_applications,
            utility_model_total=data.utility_model_total, design_total=data.design_total,
            authorized_total=data.authorized_total, new_patents_year=data.new_patents_year,
            software_copyright_total=data.software_copyright_total, trademarks_total=data.trademarks_total)
