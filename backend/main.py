...
from core.dependencies import get_db, engine
from models import EnergyData, Base, MaterialData, WaterData, EmissionData, EmploymentData, TrainingData, \
    SatisfactionData, SupplyData, WasteData, InvestmentData, OHSData, LaborReason, ProductResponsibilityData, IPRData, \
    CommunityData, VolunteerData, OtherReason, LaborQualitative, OtherQualitative, EnvQuantData, EnvQualData,ProductResponsibility,IPR,Community,Volunteer
from routers.quantitative import material
from schemas import EnergySubmission, WaterSubmission, EmissionSubmission, EmploymentSubmission, TrainingSubmission, \
    SatisfactionSubmission, SupplySubmission, WasteSubmission, InvestmentSubmission, OHSSubmission, EnvQualDataRequest,VolunteerSubmission,CommunitySubmission,IPRSubmission,ProductResponsibilitySubmission
...
async def save_labor_reasons(factory: str = Body(...), year: int = Body(...), reasons: Dict[str, str] = Body(...),
                             db: Session = Depends(get_db)):
    try:
...
async def save_other_reasons(factory: str = Body(...), year: int = Body(...), reasons: Dict[str, str] = Body(...),
                             db: Session = Depends(get_db)):
    try:
        for indicator, reason in reasons.items():
...
# Product Responsibility Submission
@app.post("/quantitative/responsibility")
async def submit_product_responsibility_data(data: ProductResponsibilitySubmission, db: Session = Depends(get_db)):
    try:
        # Calculate totals and rates based on frontend data
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"产品责任数据提交失败: {str(e)}")
    finally:
        db.close()

# Intellectual Property Rights (IPR) Submission
@app.post("/api/ipData")
async def submit_ipr_data(data: IPRSubmission, db: Session = Depends(get_db)):
    try:
        db_record = IPRData(
            factory=data.factory,
            year=data.year,
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"知识产权数据提交失败: {str(e)}")
    finally:
        db.close()

# Community and Volunteer Submission
@app.post("/quantitative/community")
async def submit_community_data(data: CommunitySubmission, volunteer_data: VolunteerSubmission, db: Session = Depends(get_db)):
    try:
        # Community Data
        ...
        volunteer_record = VolunteerData(
            factory=volunteer_data.factory,
            year=volunteer_data.year,
        ...
        return {
            "status": "success",
            "community": {"id": community_record.id, "factory": community_record.factory, "year": community_record.year},
            "volunteer": {"id": volunteer_record.id, "factory": volunteer_record.factory, "year": volunteer_record.year}
        }
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"社区参与数据提交失败: {str(e)}")
    finally:
        db.close()
